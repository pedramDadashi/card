...
public class SchedulerService {
    ...
    private void updateRestriction() {
        List<Card> expiredCardList = cardService.findAllByRestrictionNumberOfDaysLeftRestriction(1);
        if (!expiredCardList.isEmpty())
            checkExpired(expiredCardList);
        List<Card> activateCardList = cardService.findAllByRestrictionNumberOfDaysLeftRestrictionGreaterThan(1);
    ...
    private void checkExpired(List<Card> expiredCardList) {
        for (Card card : expiredCardList) {
            ...
            card.setBlock(true);
            bankAccount.setBalance(bankAccount.getBalance() + card.getCardBalance());
